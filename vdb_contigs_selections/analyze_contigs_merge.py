...
etp=[]
for e in glob.glob('/shares/CIBIO-Storage/CM/scratch/users/moreno.zolfo/virome_data/high_enrichment/vdb7/*.csv'):
...
avaFile='/shares/CIBIO-Storage/CM/scratch/users/moreno.zolfo/virome_data/high_enrichment/ava/blast_ava_2_ws13_filter.csv'
...
print ("CIRCO")
myContigs=pd.merge(myContigs,pd.read_table(metadataFile,header=0),how='left',on=['dataset','sample']).fillna('')
...
#c859-1__71__ArkhipovaK_2017__Lough_Neagh_water_sample_12__SRR4449292.orig.fna
...
thr_samples_same_as_best = 35
thr_samples_notbest_binned_otherbin = 35

thr_samples_SGB_assigned_best_and_notbest = 35
thr_samples_notbest_unbinned = 30

myContigs['prev_hits_other_dataset_distinct'] = myContigs['prev_hits_other_dataset_distinct'].replace('', np.nan).fillna(0)
#myContigs[['prev_hits_other_dataset_distinct']].to_csv('a.csv',sep='\t')
#
#sys.exit(0)
