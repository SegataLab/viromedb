...
cat ${VDB_MAIN_PATH}/virome_contigs.txt | parallel --env VDB_MAIN_PATH --env VIROMEDB_PROMISING_CONTIGS --env OUT_FOLDER -j 64 'i={}; bn=$(basename $i); sn=$(basename $(dirname $i)); dataset=$(basename $(dirname $(dirname $(dirname $i)))); echo ${dataset} ${sn} ${bn}; ${VDB_MAIN_PATH}/clustering/extract_contigs_viromes/blast_contig_against.sh $i ${dataset}__${sn}__${bn//_filtered.fasta.bz2/}';
