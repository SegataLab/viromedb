...
if [ ! -f ${odir}/sequences_Q_R.msh ]; then
	...
	/shares/CIBIO-Storage/CM/mir/tools/mash-2.0/mash sketch -i -p 36 -s 1000 -o ${odir}/sequences_Q_R ${odir}/sequences_Q_R.fna
fi;
...
if [ ! -f ${odir}/step1/nr90.fasta ]; then
	if [ $prog == "vsearch" ]; then
		...
		/shares/CIBIO-Storage/CM/mir/tools/vsearch-2.14.2/bin/vsearch --cluster_fast ${odir}/first_step.fna --threads 36 --id 0.9 --strand both --centroids ${odir}/step1/nr90.fasta --uc ${odir}/step1/clusters90.uc --maxseqlength 200000
	fi;
...
if [ ! -d ${odir}/step1/centroids/ ]; then
	...
	export ODIR=${odir};
	ls ${odir}/step1/centroids/*.fasta | parallel -j 36 --env ODIR 'fie={}; fiebn=$(basename $fie) ;echo "Mash against "${fiebn//.fasta/}; /shares/CIBIO-Storage/CM/mir/tools/mash-2.0/mash dist -d 0.1 -v 0.05 ${ODIR}/sequences_Q_R.msh ${fie} > ${ODIR}/step1/centroids/${fiebn//.fasta/.mash};'  
fi;
...
if [ ! -d ${odir}/step3_clusters/ ]; then
	...
	export ODIR=${odir};
	ls ${odir}/step2_clusters/*_full_cluster.fasta |  parallel -j 32 --env ODIR 'st3_cluster={}; echo "-- Clustering ${st3_cluster}"; st3_cluster_basename_f=$(basename $st3_cluster); st3_cluster_basename=${st3_cluster_basename_f//_full_cluster/}; if [ ! -f ${ODIR}/step3_clusters/${st3_cluster_basename}_clusters90.uc ]; then  /shares/CIBIO-Storage/CM/mir/tools/vsearch-2.13.6/bin/vsearch --cluster_fast ${st3_cluster} --threads 2 --id 0.7 --strand both --uc ${ODIR}/step3_clusters/${st3_cluster_basename}_clusters90.uc --maxseqlength 200000; fi;'

fi;

if [ ! -f ${odir}/step3_clusters/clusters_step3.tsv ]; then
	echo "Done! Now merging Step 3"
	echo python3 ./process_cluster_uc_files_step3.py --step3_folder ${odir}/step3_clusters/ --vdb_contigs ${odir}/step2_clusters/clusters_step2.tsv --allcontigs ../against_sgbs/sequences.fna ../against_viromes/sequences.fna ../promising_contigs.fna ${SEEKER_FOLDER}/promising_contigs.predict.txt
fi;
...
python3 ./unify.py --original_filtered_contigs /shares/CIBIO-Storage/CM/scratch/users/moreno.zolfo/virome_data/high_enrichment/out_toplen_filtered_largethresholds.csv --reclustered_genomes_folder ${odir}//step3_clusters/ --refseq_file /shares/CIBIO-Storage/CM/news/users/moreno.zolfo/mzolfo_virome/indexes/REFSEQ_r91/refseq_91.fasta --percentile 50 --strict --output_folder ${odir}/step4_clusters/ 
